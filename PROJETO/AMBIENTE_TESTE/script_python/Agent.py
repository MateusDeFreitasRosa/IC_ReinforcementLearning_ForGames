...
class Nintendo():
    ...
    def actions(n):
        if n > 7:
            ...
            return
        return ['up', 'down', 'left', 'right', 'A', 'B', 'start', 'select'][n]
    ...
    def preprocess_img(self, img):        
        new_image = cv2.resize(np.asarray(img / 255, dtype=np.float32), self.state_size, interpolation=cv2.INTER_AREA)
        ...
        try:
            for i_episodes in range(self.episodes):
                ...
                while not done:
                    ...
                    js = self.env.step(self.actions(action))
                    next_state = js['matriz']
                    if js['reward'] > reward:
                        reward = 1
                    elif js['reward'] == reward:
                        reward = 0
                    else:
                        reward = -1
                    done = js['endgame']
