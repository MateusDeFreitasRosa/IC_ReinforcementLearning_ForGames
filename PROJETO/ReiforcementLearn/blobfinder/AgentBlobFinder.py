# -*- coding: utf-8 -*-
"""
Created on Sun May 22 16:37:02 2022
...
class Agent(): 
    def __init__(self, state_size, action_size):
        ...
        self.n_trains                   =       0
        self.initial_start_size         =       experiment_params['params_agent']['initial_start_size']
...
#                             activation=conv['activation'], padding=conv['padding']))
    ...
    def pack_K_frames(self, sample_batch_size):
        ...
        state = np.empty((sample_batch_size, self.k_frames*self.state_size))
        action = np.empty(sample_batch_size, dtype=np.uint8)
        reward = np.empty(sample_batch_size, dtype=np.float32)
        next_state = np.empty((sample_batch_size, self.k_frames*self.state_size))
    ...
    def update_target_model(self, current_frame):
        if (current_frame % UPDATE_TARGET_MODEL == 0 and TRAIN and current_frame > self.initial_start_size):
...
class Breakout():
    ...
    def findBlobs(self, state):    
        thresh_img = cv2.threshold(np.expand_dims(np.asarray(state, dtype=np.uint8), 2), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
